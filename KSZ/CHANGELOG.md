...
- 信标讲解规则集中到 `KSZ/.env`：新增自动讲解、首次播报、切换打断开关及冷却、位置/待播有效期、交互静默参数；Compose 传递并在启动时校验，全局讲解开关和冷却同步到新协议有效策略，保留设备授权与用户交互优先。

- management 场馆资产联动：设备模式和授权范围、共享信标目录缓存、位置有效期、同信标去重及文字/视觉统一可信位置上下文；专用实现集中于 `KSZ/server`。
- MQTT 待机导览控制协议 v1：策略分块与应用确认、稳定位置心跳、按需建立语音会话、重复 hello 防重和 UDP/TTS 就绪确认；保留旧固件活动会话兼容过滤。
- 增加独立协议、运行状态机及1000设备规模验证脚本，以及 KSZ 专用服务镜像。

- 增加可选 MQTT+UDP 网关部署 profile，固定上游源码与依赖锁；覆盖唤醒后的 KSZ 语音、语言换绑、信标导览、MCP 拍照和视觉流式传输，附 OTA 检查及隔离协议测试。
- MQTT 网关通过 KSZ 补丁直接透传 MCP 初始化和工具发现，保留设备专属视觉配置；认证成功后才接受会话，断开 MQTT 时同步清理后端连接。
- 视觉流式播报：OpenAI 兼容 VLLM 读取 SSE 正文增量并按句提交 TTS，字幕随对应音频发送，不经过聊天 LLM 二次改写。
- 增加可选 `vision_stream` 能力及 `request_id` / `delivery_mode` 协议，区分 MCP、手动直推和仅 HTTP 返回；数字人页面增加手动拍照入口与取消回执，旧端保留完整返回兼容。

### 变更

- 信标自动讲解改为固定位置开场加模型正文：四语模板集中到 `KSZ/.env`，地点句即时进入同一轮 TTS；导览专用流式过滤覆盖普通回复、`direct_answer` 和工具续答，移除开头问候、自我介绍和重复位置，并统一字幕、历史记录与中断处理。
...
- 自动讲解改为按有效信标 MAC 变化触发，同一导览区域内切换信标也会讲解；保留首次定位建立基准、同信标去重、播报冷却及用户交互优先，并以最新信标替换待播任务。
- 信标自动讲解与普通回答使用一致的抢占语义：切换到新位置时中止仍在播放的旧信标讲解、清理旧轮次并立即讲解最新位置；用户主动对话仍优先，不被信标强行打断。

- 自动讲解直接启动语音输出，内部位置提示不再作为用户识别文本回传设备；补充固件握手后主动建立UDP回程的要求。
- 管理端设备在线状态改由认证后的 MQTT/WebSocket 连接独立同步，支持旧固件待机在线、断线与重连；修复未登记策略缓存阻止设备自动发现，并增加独立在线同步开关。
- MQTT 成功 CONNACK 延后至签名验证通过，拒绝同一连接重复 CONNECT，避免认证失败或重复握手留下错误在线记录。
- MQTT 手动拾音等待模拟设备声明的 UDP 末序列到达，并记录服务端实际提交给 ASR 的音频时长。
- 手动拾音模式在连接入口直接累计 PCM，避免 `listen stop` 越过异步 VAD 队列而只提交开头音频；MQTT 网关在转发停止消息前等待 UDP 尾包稳定。
- 补齐兼容模式手动拍照直推的 `tts:start` 与服务端播放状态，启动通知发送完成后才提交语音；等待期间断线、打断或换轮不播放旧结果，MCP 拍照保持去重。
- 百炼实际收到文本才开启合成任务；工具等待提示及完整视觉句子各自结束供应商任务，同一设备轮次仅结束一次，避免等待识别、句间停顿及延迟 MCP 回执造成空闲任务超时。
- 修复 MCP 相机成功纯文本被误判失败；成功结果直答，异常统一安全提示，部分正文后中断仅提示一次，取消静默，原始错误不进入聊天 LLM 或 TTS。
- 新版视觉请求按设备、请求编号和轮次关联，新照片替换旧识别，打断与断线丢弃迟到结果；视觉与 MCP 工具等待共用 120 秒截止时间，避免独立 30 秒超时及重复播报。
